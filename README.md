...
- **Per-sandbox long-term memory** — Every prompt sent to a sandbox is stored in Supermemory alongside the files it changed, tagged to that specific sandbox. This builds up a persistent knowledge base for each sandbox over time =across sessions, container restarts, and deployments.
- **RAG-powered AI context** — Before every AI call, Supermemory is queried with the current prompt to retrieve the most relevant past changes for that sandbox. Those results are injected into the AI's context window, so the model understands what has already been changed and why, allowing for coherent, non-repetitive edits that build on pri-or work rather than starting from scratch each time.
