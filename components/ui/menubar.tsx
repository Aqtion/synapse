...
function Menubar({
  ...
  return (
    <MenubarPrimitive.Root
      data-slot="menubar"
      className={cn("bg-background h-8 gap-0.5 rounded-sm border p-[3px] flex items-center", className)}
      {...props}
    />
  )
}
...
function MenubarSubContent({
  ...
  return (
    <MenubarPrimitive.SubContent
      ...
      className={cn("bg-popover text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 min-w-32 rounded-sm p-1 shadow-lg ring-1 duration-100 z-50 origin-(--radix-menubar-content-transform-origin) overflow-hidden", className )}
      {...props}
    />
  )
}
